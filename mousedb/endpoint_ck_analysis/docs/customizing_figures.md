...
| Parameter | What it controls |
|-----------|------------------|
| `USE_SYNTHETIC` | If True, swap real data for a cloned-and-perturbed synthetic cohort. Used to validate the pipeline at a larger N while the real cohort is still small. |
...
| `N_CLUSTERS` | Target cluster count. At real small-N this defaults to 4 (one per subject). |
...
| Parameter | What it controls |
|-----------|------------------|
| `USE_SYNTHETIC` / `SYNTHETIC_N` / `SYNTHETIC_SEED` | Same meaning as in notebook 07; at small N the nested LMM section is underdetermined, so synthetic mode is the right test bench. |
