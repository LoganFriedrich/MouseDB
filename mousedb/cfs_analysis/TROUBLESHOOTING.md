...
## "No module named 'cfs_analysis'" in a notebook cell

The kernel you selected is not the `Python (cfs_analysis)` kernel.
...
(cfs_analysis)** from the dropdown.
...
`cfs_analysis/_bundled_data/`. The file should be at
`.../cfs_analysis/cfs_analysis/_bundled_data/connectome.db` (note the
nested folder name).
...
```
set CFS_ANALYSIS_DB=Y:\LAB_ROOT\Databases\connectome.db
run_analysis.bat
```
...
is `cfs_analysis/helpers/`; notebook code is intentionally thin and calls
into those modules.
